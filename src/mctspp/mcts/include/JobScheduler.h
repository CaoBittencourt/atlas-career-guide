...
class Job {  // extend this to whatever job you want scheduled (in a different
             // .cpp/.h)
 public:
...
struct thread_args {
  ...
  thread_args(queue<Job *> *_jobQueue, pthread_mutex_t *_queueLock,
              pthread_cond_t *_queueCond, volatile unsigned int *_jobsRunning,
              volatile bool *_threads_must_exit,
              pthread_cond_t *_jobs_finished_cond,
              unordered_map<int, volatile unsigned int> *_tagged_jobs_pending)
      : jobQueue(_jobQueue),
        queueLock(_queueLock),
        queueCond(_queueCond),
        jobsRunning(_jobsRunning),
        threads_must_exit(_threads_must_exit),
        jobs_finished_cond(_jobs_finished_cond),
        tagged_jobs_pending(_tagged_jobs_pending) {}
};
...
class JobScheduler {
  ...
  unordered_map<int, volatile unsigned int>
      tagged_jobs_pending;  // tag -> number of jobs
 public:
