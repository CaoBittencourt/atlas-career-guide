...
/*** MCTS NODE ***/
MCTS_node::MCTS_node(MCTS_node *parent, MCTS_state *state,
                     const MCTS_move *move)
    : parent(parent),
      state(state),
      move(move),
      score(0.0),
...
MCTS_node::~MCTS_node() {
  ...
  while (!untried_actions->empty()) {
    delete untried_actions
        ->front();  // if a move is here then it is not a part of a child node
                    // and needs to be deleted here
    untried_actions->pop();
  }
  delete untried_actions;
}
...
void MCTS_node::expand() {
  if (is_terminal()) {  // can legitimately happen in end-game situations
    rollout();  // keep rolling out, eventually causing UCT to pick another node
                // to expand due to exploration
    return;
...
void MCTS_node::rollout() {
  ...
  // schedule Jobs
  static JobScheduler
      scheduler;  // static so that we don't create new threads every time (!)
  ...
  for (int i = 0; i < NUMBER_OF_THREADS; i++) {
    if (results[i] >= 0.0 && results[i] <= 1.0) {
      ...
      cerr << "Warning: Invalid result when aggregating parallel rollouts"
           << endl;
    }
  }
...
bool MCTS_node::is_terminal() const { return terminal; }

unsigned int MCTS_node::get_size() const { return size; }
...
MCTS_node *MCTS_node::select_best_child(double c) const {
  ...
  if (children->empty())
    ...
    for (auto *child : *children) {
      ...
      // If its the opponent's move apply UCT based on his winrate i.e. our loss
      // rate.   <-------
      if (!state->player1_turn()) {
        winrate = 1.0 - winrate;
      }
      if (c > 0) {
        uct = winrate + c * sqrt(log((double)this->number_of_simulations) /
                                 ((double)child->number_of_simulations));
      } else {
        uct = winrate;
      }
      if (uct > max) {
...
MCTS_node *MCTS_node::advance_tree(const MCTS_move *m) {
  ...
  // remove children from queue so that they won't be re-deleted by the
  // destructor when this node dies (!)
  this->children->clear();
  // if not found then we have to create a new node
  if (next == NULL) {
    // Note: UCT may lead to not fully explored tree even for short-term
    // children due to terminal nodes being chosen
    ...
    // IMPORTANT: m and next->move can be the same here if we pass the move from
    // select_best_child() (which is what we will typically be doing). If not
    // then it's the caller's responsibility to delete m (!)
  }
...
MCTS_tree::~MCTS_tree() { delete root; }
...
#ifdef DEBUG
      cout << "Early stopping: Made " << (i + 1) << " iterations in " << dt
           << " seconds." << endl;
#endif
      break;
    }
...
unsigned int MCTS_tree::get_size() const { return root->get_size(); }

const MCTS_move *MCTS_node::get_move() const { return move; }
...
#define TOPK 10
  ...
  cout << "___ INFO _______________________" << endl
       ...
       << "Chances of P1 winning: " << setprecision(4)
       << 100.0 * (score / number_of_simulations) << "%" << endl;
  // sort children based on winrate of player's turn for this node (!)
  if (state->player1_turn()) {
    std::sort(children->begin(), children->end(),
              [](const MCTS_node *n1, const MCTS_node *n2) {
                return n1->calculate_winrate(true) >
                       n2->calculate_winrate(true);
              });
  } else {
    std::sort(children->begin(), children->end(),
              [](const MCTS_node *n1, const MCTS_node *n2) {
                return n1->calculate_winrate(false) >
                       n2->calculate_winrate(false);
              });
  }
  ...
  for (int i = 0; i < children->size() && i < TOPK; i++) {
    cout << "  " << i + 1 << ". " << children->at(i)->move->sprint()
         << "  -->  " << setprecision(4)
         << 100.0 * children->at(i)->calculate_winrate(state->player1_turn())
         << "%" << endl;
  }
...
void MCTS_tree::advance_tree(const MCTS_move *move) {
  ...
  delete old_root;  // this won't delete the new root since we have emptied
                    // old_root's children
}

const MCTS_state *MCTS_tree::get_current_state() const {
...
/*** MCTS agent ***/
MCTS_agent::MCTS_agent(MCTS_state *starting_state, int max_iter,
                       int max_seconds)
...
const MCTS_move *MCTS_agent::genmove(const MCTS_move *enemy_move) {
  ...
  if (best_child == NULL) {
    cerr << "Warning: Tree root has no children! Possibly terminal node!"
         << endl;
    return NULL;
  }
...
MCTS_agent::~MCTS_agent() { delete tree; }

const MCTS_state *MCTS_agent::get_current_state() const {
