...
#define CHECK_PERROR(call, msg, actions) \
  {                                      \
    if ((call) < 0) {                    \
      perror(msg);                       \
      actions                            \
...
JobScheduler::JobScheduler(unsigned int _number_of_threads)
    : jobs_running(0), number_of_threads(_number_of_threads), t_args(NULL) {
  threads_must_exit = false;
  CHECK_PERROR(pthread_mutex_init(&queue_lock, NULL),
               "pthread_mutex_t_init failed", )
  CHECK_PERROR(pthread_cond_init(&queue_cond, NULL),
               "pthread_cond_init failed", )
  CHECK_PERROR(pthread_cond_init(&jobs_finished_cond, NULL),
               "pthread_cond_init failed", )
  t_args = new struct thread_args(&job_queue, &queue_lock, &queue_cond,
                                  &jobs_running, &threads_must_exit,
                                  &jobs_finished_cond, &tagged_jobs_pending);
  threads = new pthread_t[number_of_threads];
  for (int i = 0; i < number_of_threads; i++) {
    CHECK_PERROR(pthread_create(&threads[i], NULL, thread_code, (void *)t_args),
                 "pthread_create failed", threads[i] = 0;)
  }
}
...
JobScheduler::~JobScheduler() {
  ...
  CHECK_PERROR(pthread_cond_broadcast(&queue_cond),
               "pthread_broadcast failed", )
  for (int i = 0; i < number_of_threads; i++) {
    CHECK_PERROR(pthread_join(threads[i], NULL), "pthread_join failed", )
  }
  delete[] threads;
  CHECK_PERROR(pthread_mutex_destroy(&queue_lock),
               "pthread_mutex_destroy failed", )
  CHECK_PERROR(pthread_cond_destroy(&queue_cond),
               "pthread_cond_destroy failed", )
  CHECK_PERROR(pthread_cond_destroy(&jobs_finished_cond),
               "pthread_cond_destroy failed", )
}
...
void JobScheduler::schedule(Job *job) {
  ...
  if (job->TAG != NOTAG) {
    ...
    if (it == tagged_jobs_pending.end())
      tagged_jobs_pending.insert(make_pair(job->TAG, 1));
    else
      (it->second)++;
  }
  job_queue.push(job);
  CHECK_PERROR(pthread_cond_signal(&queue_cond), "pthread_cond_signal failed", )
  CHECK_PERROR(pthread_mutex_unlock(&queue_lock),
...
void JobScheduler::waitUntilJobsHaveFinished(int tag) {
  if (tag == NOTAG) {
    ...
    while (!job_queue.empty() || jobs_running > 0) {
      CHECK_PERROR(pthread_cond_wait(&jobs_finished_cond, &queue_lock),
...
void *thread_code(void *args) {
  ...
  unordered_map<int, volatile unsigned int> *tagged_jobs_pending_ptr =
      argptr->tagged_jobs_pending;
  ...
  while (!(*threads_must_exit_ptr)) {
    ...
    while (job_queue->empty() && !(*threads_must_exit_ptr)) {
      CHECK_PERROR(pthread_cond_wait(queue_cond, queue_lock),
                   "pthread_cond_wait failed", )
    }
    ...
    tag = job->TAG;
    job->run();
    delete job;  // (!) scheduler deletes scheduled objects when done but they
                 // must be allocated before they get scheduled
    ...
    if (num == 0 || (*jobs_running_ptr) == 0) {
      CHECK_PERROR(pthread_cond_broadcast(jobs_finished_cond_ptr),
