...
/** TODOs-Ideas:
 * - If all I need is the shortest path to a goal state then A* search could be
 * faster than BFS with a heuristic that prioritizes moves towards the endzone
 * (i.e. up for black, down for white)?
 */
...
struct Quoridor_move : public MCTS_move {
  ...
  Quoridor_move(short int x, short int y, char p, char t)
      : x(x), y(y), player(p), type(t) {}
  ...
  string sprint() const override {
    string movetype = (type == 'h')   ? "places horizontal wall at"
                      : (type == 'v') ? "places vertical wall at"
                                      : "moves to";
    string playerstr = (player == 'W') ? "White" : "Black";
    return playerstr + " " + movetype + " " + string(1, (char)('A' + y)) +
           to_string(x + 1);
  }
};
...
class Quoridor_state : public MCTS_state {
   ...
   * ' ', 'h', 'v', 'b' for empty, horizontal, vertical and both walls in the
   * right and bottom part of a cell */
  char walls[9][9]{};
  bool wall_connections[8][8]{};  // Note: can be a bitmap to save up space
                                  // (bools are 1 byte = 8 bits)
  ...
  bool horizontal_wall(short int x, short int y) const {
    return walls[x][y] == 'h' || walls[x][y] == 'b';
  }
  bool vertical_wall(short int x, short int y) const {
    return walls[x][y] == 'v' || walls[x][y] == 'b';
  }
  ...
  bool legal_wall(short int x, short int y, char p, bool horizontal,
                  bool check_blocking = true);
  short int **calculate_dists_from(short int x, short int y, bool stop_at_goal,
                                   char player);
 ...
 public:
  ...
  short int remaining_walls(char p) const {
    return (p == 'W') ? wwallsno : bwallsno;
  }
  bool legal_move(const Quoridor_move *move);
  bool play_move(const Quoridor_move *move);
  int get_shortest_path(char player,
                        const Quoridor_move *extra_wall_move = NULL,
                        short int posx = -1, short int posy = -1);
  ...
  friend Quoridor_move *pick_semirandom_move(
      Quoridor_state &s, std::uniform_real_distribution<double> &dist,
