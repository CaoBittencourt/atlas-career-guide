...
#define TEST_ALL_MOVES  // test all moves vs just some found good by a heuristic
                        // (increases branching factor of tree but could find
                        // unexpectedly good moves)
#define MAX(A, B) (((A) > (B)) ? A : B)

using namespace std;

default_random_engine Quoridor_state::generator =
...
Quoridor_state::Quoridor_state(const Quoridor_state &other)
    : move_counter(other.move_counter),
      ...
      bx(other.bx),
      by(other.by),
      wwallsno(other.wwallsno),
      bwallsno(other.bwallsno),
      turn(other.turn),
      wdists(NULL),
      bdists(NULL) {  // TODO: Is it cheaper to copy dists than to potentially
                      // recalculate them?
  for (int i = 0; i < 81; i++) {
...
short int **Quoridor_state::calculate_dists_from(short int x, short int y,
                                                 ...
                                                 char player) {
  /** Important note:
   * - stop_at_goal=true should be used when we only care about 1 square of the
   * endzone (the minimum one) being calculated and we don't really care for the
   * rest of the board (which is typically the case)
   * - In that case we also need the player parameter. Otherwise it won't be
   * used.
   */
  if (x < 0 || x >= 9 || y < 0 || y >= 9) {
    cerr << "Error: Invalid coordinates in calculate_dists_from()"
         << endl;  // should not happen
    return NULL;
  }
  ...
  while (!Q.empty()) {
    ...
    if (n.x - 1 >= 0 && !horizontal_wall(n.x - 1, n.y) &&
        dists[n.x - 1][n.y] < 0) {  // up
      dists[n.x - 1][n.y] = n.dist + 1;
      if (stop_at_goal && is_goal_state(player, n.x - 1)) break;
      Q.push(Node(n.x - 1, n.y, n.dist + 1));
    }
    if (n.x + 1 < 9 && !horizontal_wall(n.x, n.y) &&
        dists[n.x + 1][n.y] < 0) {  // down
      dists[n.x + 1][n.y] = n.dist + 1;
      if (stop_at_goal && is_goal_state(player, n.x + 1)) break;
      Q.push(Node(n.x + 1, n.y, n.dist + 1));
    }
    if (n.y - 1 >= 0 && !vertical_wall(n.x, n.y - 1) &&
        dists[n.x][n.y - 1] < 0) {  // left
      dists[n.x][n.y - 1] = n.dist + 1;
      Q.push(Node(n.x, n.y - 1, n.dist + 1));
    }
    if (n.y + 1 < 9 && !vertical_wall(n.x, n.y) &&
        dists[n.x][n.y + 1] < 0) {  // right
      dists[n.x][n.y + 1] = n.dist + 1;
      Q.push(Node(n.x, n.y + 1, n.dist + 1));
    }
  }
...
int Quoridor_state::get_shortest_path(char player,
                                      const Quoridor_move *extra_wall_move,
                                      short int posx, short int posy) {
  ...
  // if dists is NULL then we need to re-calculate dists separately
  // (disregarding previous value)
  if (extra_wall_move != NULL) {
    ...
    if (!legal_wall(
            extra_wall_move->x, extra_wall_move->y, extra_wall_move->player,
            extra_wall_move->type == 'h',
            false)) {  // (!) check_blocking = false to prevent infinite loop
    ...
    add_wall(extra_wall_move->x + ((int)!horizontal),
             extra_wall_move->y + ((int)horizontal), horizontal);
    // calc dists
    dists = calculate_dists_from(posx, posy, true, player);
    // remove wall
    remove_wall(extra_wall_move->x, extra_wall_move->y, horizontal);
    remove_wall(extra_wall_move->x + ((int)!horizontal),
                extra_wall_move->y + ((int)horizontal), horizontal);
...
void Quoridor_state::add_wall(short int x, short int y, bool horizontal) {
  // Note: this low-level function does NOT reset dists calculated so we need to
  // do that outside if we wish so
  char put = horizontal ? 'h' : 'v', other = horizontal ? 'v' : 'h';
  if (walls[x][y] == ' ')
    walls[x][y] = put;
  else if (walls[x][y] == other)
    walls[x][y] = 'b';
  else {
    cerr << "Warning: Illegal wall let through!" << endl;
  }  // should not happen
}
...
bool Quoridor_state::legal_step(short int x, short int y, char p) const {
  ...
  if (y == posy) {
    if (x >= 0 && !horizontal_wall(posx - 1, posy)) {
      ...
      if (x == posx - 2 && !horizontal_wall(posx - 2, posy) &&
          enemy_posy == posy && enemy_posx == posx - 1)
        return true;  // up-up
    }
  ...
  if (x == posx) {
    if (y >= 0 && !vertical_wall(posx, posy - 1)) {
      ...
      if (y == posy - 2 && !vertical_wall(posx, posy - 2) &&
          enemy_posx == posx && enemy_posy == posy - 1)
  ...
  if (x >= 0 && y >= 0 && x == posx - 1 && y == posy - 1) {  // up-left
    if (enemy_posx == posx - 1 && enemy_posy == posy &&
        ...
        !vertical_wall(posx - 1, posy - 1) && !horizontal_wall(posx - 1, posy))
      return true;
    if (enemy_posx == posx && enemy_posy == posy - 1 &&
        (posy - 2 < 0 || vertical_wall(posx, posy - 2)) &&
        !horizontal_wall(posx - 1, posy - 1) && !vertical_wall(posx, posy - 1))
      return true;
  }
  if (x >= 0 && y < 9 && x == posx - 1 && y == posy + 1) {  // up-right
    if (enemy_posx == posx - 1 && enemy_posy == posy &&
        (posx - 2 < 0 || horizontal_wall(posx - 2, posy)) &&
        !vertical_wall(posx - 1, posy) && !horizontal_wall(posx - 1, posy))
      return true;
    if (enemy_posx == posx && enemy_posy == posy + 1 &&
        (posy + 2 >= 9 || vertical_wall(posx, posy + 1)) &&
        !horizontal_wall(posx - 1, posy + 1) && !vertical_wall(posx, posy))
      return true;
  }
  if (x < 9 && y >= 0 && x == posx + 1 && y == posy - 1) {  // down-left
    if (enemy_posx == posx + 1 && enemy_posy == posy &&
        (posx + 2 >= 9 || horizontal_wall(posx + 1, posy)) &&
        !vertical_wall(posx + 1, posy - 1) && !horizontal_wall(posx, posy))
      return true;
    if (enemy_posx == posx && enemy_posy == posy - 1 &&
        (posy - 2 < 0 || vertical_wall(posx, posy - 2)) &&
        !horizontal_wall(posx, posy - 1) && !vertical_wall(posx, posy - 1))
      return true;
  }
  if (x < 9 && y < 9 && x == posx + 1 && y == posy + 1) {  // down-right
    if (enemy_posx == posx + 1 && enemy_posy == posy &&
        (posx + 2 >= 9 || horizontal_wall(posx + 1, posy)) &&
        !vertical_wall(posx + 1, posy) && !horizontal_wall(posx, posy))
      return true;
    if (enemy_posx == posx && enemy_posy == posy + 1 &&
        (posy + 2 >= 9 || vertical_wall(posx, posy + 1)) &&
        !horizontal_wall(posx, posy + 1) && !vertical_wall(posx, posy))
      return true;
  }
  return false;
}

bool Quoridor_state::legal_wall(short int x, short int y, char p,
                                bool horizontal, bool check_blocking) {
  // check if our turn
  if (p != turn) return false;
  // check out-of-bounds
  ...
  // check if blocked by the same wall or by an opposite wall at the same exact
  // spot
  char type = (horizontal) ? 'h' : 'v';
  char opposite_type = (horizontal) ? 'v' : 'h';
  if (walls[x][y] == type || walls[x][y] == 'b' ||
      (walls[x][y] == opposite_type && wall_connections[x][y]))
    return false;
  ...
  // check if playing this wall blocks a pawn's path (expensive, avoid when
  // possible)
  if (check_blocking) {
    // TODO: if there are no walls/edges in both sides then there is no way this
    // wall closed any paths.. -> don't bfs But that is very hard to check so
    // instead just check for completely isolated ones:
    bool isolated = true;
...
bool Quoridor_state::play_move(const Quoridor_move *move) {
  if (move == NULL || !legal_move(move)) {
    cout << "Invalid command: Illegal move: "
         << ((move != NULL) ? move->sprint() : "NULL") << endl
         << endl;
    return false;
  }
  if (move->type == 'h' || move->type == 'v') {  // wall move
...
#define BOTH "╬"
  ...
  cout << "           Player turn: " << (turn == 'W' ? "White" : "Black")
       << endl;
  cout << "    +";
  for (int i = 0; i < 9; i++) {
    cout << " ━━━ +";
  ...
  for (int row = 0; row < 9; row++) {
    ...
    // print wall counts
    if (row < 2) {
      printf("     %s walls: %d", (row == 0) ? "White" : "Black",
             (row == 0) ? wwallsno : bwallsno);
    }
    cout << endl << "    +";
    for (int col = 0; col < 9; col++) {
      if (horizontal_wall(row, col)) {
        printf("═════%s", wall_connections[row][col]
                              ? ((row < 8 && vertical_wall(row, col) &&
                                  wall_connections[row][col] &&
                                  vertical_wall(row + 1, col))
                                     ? VWALL
                                     : "═")
                              : "+");
      } else if (row < 8) {
        printf("-----%s",
               (vertical_wall(row, col) && wall_connections[row][col] &&
                vertical_wall(row + 1, col))
                   ? VWALL
                   : "+");
      } else {
...
forward_list<MCTS_move *> Quoridor_state::get_legal_step_moves(char p) const {
  ...
  if (legal_step(posx - 1, posy, p))
    Q.push_front(new Quoridor_move(posx - 1, posy, p, ' '));
  if (legal_step(posx - 2, posy, p))
    Q.push_front(new Quoridor_move(posx - 2, posy, p, ' '));
  if (legal_step(posx + 1, posy, p))
    Q.push_front(new Quoridor_move(posx + 1, posy, p, ' '));
  if (legal_step(posx + 2, posy, p))
    Q.push_front(new Quoridor_move(posx + 2, posy, p, ' '));
  if (legal_step(posx, posy - 1, p))
    Q.push_front(new Quoridor_move(posx, posy - 1, p, ' '));
  if (legal_step(posx, posy - 2, p))
    Q.push_front(new Quoridor_move(posx, posy - 2, p, ' '));
  if (legal_step(posx, posy + 1, p))
    Q.push_front(new Quoridor_move(posx, posy + 1, p, ' '));
  if (legal_step(posx, posy + 2, p))
    Q.push_front(new Quoridor_move(posx, posy + 2, p, ' '));
  if (legal_step(posx - 1, posy - 1, p))
    Q.push_front(new Quoridor_move(posx - 1, posy - 1, p, ' '));
  if (legal_step(posx - 1, posy + 1, p))
    Q.push_front(new Quoridor_move(posx - 1, posy + 1, p, ' '));
  if (legal_step(posx + 1, posy - 1, p))
    Q.push_front(new Quoridor_move(posx + 1, posy - 1, p, ' '));
...
vector<MCTS_move *> Quoridor_state::get_legal_step_moves2(char p) const {
  ...
  short int posy = (turn == 'W') ? wy : by;
  if (legal_step(posx - 1, posy, p))
    Q.push_back(new Quoridor_move(posx - 1, posy, p, ' '));
  if (legal_step(posx - 2, posy, p))
    Q.push_back(new Quoridor_move(posx - 2, posy, p, ' '));
  if (legal_step(posx + 1, posy, p))
    Q.push_back(new Quoridor_move(posx + 1, posy, p, ' '));
  if (legal_step(posx + 2, posy, p))
    Q.push_back(new Quoridor_move(posx + 2, posy, p, ' '));
  if (legal_step(posx, posy - 1, p))
    Q.push_back(new Quoridor_move(posx, posy - 1, p, ' '));
  if (legal_step(posx, posy - 2, p))
    Q.push_back(new Quoridor_move(posx, posy - 2, p, ' '));
  if (legal_step(posx, posy + 1, p))
    Q.push_back(new Quoridor_move(posx, posy + 1, p, ' '));
  if (legal_step(posx, posy + 2, p))
    Q.push_back(new Quoridor_move(posx, posy + 2, p, ' '));
  if (legal_step(posx - 1, posy - 1, p))
    Q.push_back(new Quoridor_move(posx - 1, posy - 1, p, ' '));
  if (legal_step(posx - 1, posy + 1, p))
    Q.push_back(new Quoridor_move(posx - 1, posy + 1, p, ' '));
  if (legal_step(posx + 1, posy - 1, p))
    Q.push_back(new Quoridor_move(posx + 1, posy - 1, p, ' '));
 ...
 *  This minimizes the branching factor of the search tree while also not
 * investing in subtrees caused by bad enemy (and also ours) moves, where we
 * would probably be better anyway. Although, that is addressed by UCT as well.
 */
    ...
    for (short int i = 0; i < 8; i++) {
      for (short int j = 0; j < 8; j++) {
        for (short int k = 0; k < 2; k++) {  // orientation
          if (legal_wall(i, j, p, k == 0,
                         false)) {  // cheap version (don't double count)
            // First (!), check if this walls encumbers our enemy more than us
            Quoridor_move *wallmove =
                new Quoridor_move(i, j, p, (k == 0) ? 'h' : 'v');
            ...
            // must annoy the enemy more than us (and be legal when it comes to
            // blocking)
            ...
            // Then, if this encumbers us significantly more than the enemy
            // check for counter-walls
            if (remaining_walls(enemy) > 0 &&
                our_enc - enemy_enc >= MIN_ENC_FOR_STOPPING_ENEMY_WALLS) {
              // same pos, opposite orientation (!)
              Quoridor_move *countermove =
                  new Quoridor_move(i, j, p, (k == 0) ? 'v' : 'h');
              if (!already_used[i][j][1 - k] &&
                  legal_move(countermove)) {  // also checks blocking
              ...
              if (k == 0) {
                if (j - 1 >= 0 && !already_used[i][j - 1][k] &&
                    legal_wall(i, j - 1, p, true)) {
                  countermove = new Quoridor_move(i, j - 1, p, 'h');
                  already_used[i][j - 1][k] = true;
                  Q->push(countermove);
                }
                if (j + 1 < 8 && !already_used[i][j + 1][k] &&
                    legal_wall(i, j + 1, p, true)) {
                  countermove = new Quoridor_move(i, j + 1, p, 'h');
                  already_used[i][j + 1][k] = true;
                  Q->push(countermove);
                ...
                if (i - 1 >= 0 && !already_used[i - 1][j][k] &&
                    legal_wall(i - 1, j, p, false)) {
                  countermove = new Quoridor_move(i - 1, j, p, 'v');
                  already_used[i - 1][j][k] = true;
                  Q->push(countermove);
                }
                if (i + 1 < 8 && !already_used[i + 1][j][k] &&
                    legal_wall(i + 1, j, p, false)) {
                  countermove = new Quoridor_move(i + 1, j, p, 'v');
                  already_used[i + 1][j][k] = true;
                  Q->push(countermove);
...
queue<MCTS_move *> *Quoridor_state::actions_to_try() const {
  /** Note: actions_to_try() should probably be const in superclass but it would
   * be very inefficient to be so here because we would need to recalculate
   * paths every time! This is a hack to avoid const error in this specific
   * case. */
...
#define ROOM_FOR_ERROR \
  1  // Note: Allow more room for error? path doesn't take "jumping" moves into
     // account...

  int white_path = s.get_shortest_path('W');
  int black_path = s.get_shortest_path('B');

  // if opponent is out of walls and we have the shortest path then we are
  // almost guaranteed to win
  if (s.bwallsno <= 0 && white_path + ((int)(s.whose_turn() != 'W')) <=
                             black_path - ROOM_FOR_ERROR) {
    return GUESS_WIN_CONF;
  }
  if (s.wwallsno <= 0 && black_path + ((int)(s.whose_turn() != 'B')) <=
                             white_path - ROOM_FOR_ERROR) {
    return 1.0 - GUESS_WIN_CONF;
  }
  // if opponent is almost out of walls and we have walls to stop him then we
  // are probably going to win
  if (s.bwallsno <= 1 && s.wwallsno >= 2 &&
      white_path + ((int)(s.whose_turn() != 'W')) <=
          black_path - ROOM_FOR_ERROR) {
    return GUESS_WIN_CONF - 0.1;
  }
  if (s.wwallsno <= 1 && s.bwallsno >= 2 &&
      black_path + ((int)(s.whose_turn() != 'B')) <=
          white_path - ROOM_FOR_ERROR) {
  ...
  if (max > 0)
    wallsdiff_metric =
        ((s.wwallsno > s.bwallsno) ? +1 : -1) *
        (((double)pow(s.wwallsno - s.bwallsno, 2)) / ((double)pow(max, 2)));

  /** Shortest distance heuristic
   * - After some lead it doesn't matter if we get even more ahead, we get the
   * full bonus --> keep your walls? */
  double path_diff =
      black_path - white_path +
      (s.whose_turn() == 'W' ? +0.5
                             : -0.5);  // bonus for whose turn it is to play
...
bool force_playwall(Quoridor_state &s) {
  ...
  if (enemy_path <= 2 && our_path > enemy_path + 1 && our_walls >= enemy_walls)
    return true;
  if (enemy_path <= 3 && our_path > enemy_path + 2 && our_walls > enemy_walls)
    return true;
  return false;
}
...
#define GUIDED_RANDOM_WALL 0.75
  ...
  // TODO: simulations need to play smarter walls because otherwise the AI don't
  // think of them as a threat although they are in the branching factor so they
  // really really should think of them...
  ...
  double wall_vs_move_prob = (s.get_number_of_turns() <= 2) ? 0.0
                             : (s.get_number_of_turns() <= 6)
                                 ? (WALL_VS_MOVE_CHANCE / 2)
                                 : WALL_VS_MOVE_CHANCE;

  if (s.remaining_walls(s.whose_turn() > 0 &&
                        (force_playwall(s) || dist(gen) < wall_vs_move_prob))) {
     ...
     *   and if so play it, else continue searching. If no good move was found
     * return random one or step move.
     */
    // TODO: A wall could be good in other ways as well e.g. blocks an enemy
    // good wall. How do we consider those cheaply? play wall
    vector<Quoridor_move *> pool;
    pool.reserve(128);
    for (short int i = 0; i < 8; i++) {
      for (short int j = 0; j < 8; j++) {
        if (s.legal_wall(i, j, p, true,
                         false)) {  // cheap checks (no check for blocking)
          pool.push_back(new Quoridor_move(i, j, p, 'h'));
        }
        if (s.legal_wall(i, j, p, false,
                         false)) {  // cheap checks (no check for blocking)
    ...
    if (dist(gen) < BEST_WALLMOVE) {
      /** Note: random shuffling should make the choice of maximum random in
       * case of multiple equivalents **/
      // check all wallmove's enc and pick the best (as in maximum difference in
      // encumbrances)
      ...
      for (Quoridor_move *move : pool) {
        ...
        if (enemy_path >= 0 && enemy_enc > 0) {
          ...
          if (our_path >= 0 &&
              enemy_enc > our_enc) {  // must annoy the enemy more than us
...
#define MAXSTEPS 50
#define EVALUATION_THRESHOLD \
  0.8  // when eval is this skewed then don't simulate any more, return eval
  // #define DDEBUG

  uniform_real_distribution<double> dist(0.0, 1.0);
  Quoridor_state s(*this);  // copy current state (bypasses const restriction
                            // and allows to change state)
  bool noerror;
#ifdef DDEBUG
  ...
  for (int i = 0; i < MAXSTEPS; i++) {
    ...
    if (!s.legal_move(m)) {
      cout << "Picked illegal move: " << ((m != NULL) ? m->sprint() : "NULL")
           << " intentionally! Move history:" << endl;
#ifdef DDEBUG
      while (!hist.empty()) {
