...
char parse_type(const string &s) {
  if (s == "H" || s == "h" || s == "Horizontal" || s == "horizontal" ||
      s == "HORIZONTAL")
    return 'h';
  if (s == "V" || s == "v" || s == "Vertical" || s == "vertical" ||
      s == "VERTICAL")
    return 'v';
  return 0x00;
}
...
int main() {
  ...
  MCTS_tree *game_tree =
      new MCTS_tree(new Quoridor_state());  // Important: do not use the same
                                            // state that we change in main loop

  cout << (state->whose_turn() == 'W' ? "White's move:" : "Black's move:")
       << endl
       << PROMPT;
  flush(cout);
  while (cin >> command) {
    if (command == "q" || command == "quit") {
      ...
      if (winner != ' ') {
        ...
        } else if (state->remaining_walls(
                       state->whose_turn() == 'W' ? 'B' : 'W') == 0) {
          // if enemy can't play a wall then less thinking is probably required
          // as well
        ...
        if (best_child == NULL) {
          cerr << "Warning: Could not find best child. Tree has no children? "
                  "Possible terminal node"
               << endl
               << endl;
        }
        const Quoridor_move *best_move =
            (const Quoridor_move *)best_child->get_move();
        ...
        if (!succ) {
          cerr << "Warning: AI generated illegal move: " << best_move->sprint()
               << endl
               << endl;
        } else {
          // print AI's move
      ...
      cout << "Rollout average score: " << setprecision(4) << 100.0 * score
           << endl;
    } else {
