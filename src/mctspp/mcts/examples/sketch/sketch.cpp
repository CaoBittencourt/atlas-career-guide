...
bool TicTacToe_state::is_terminal() const { return winner != ' '; }

char TicTacToe_state::get_turn() const { return turn; }

char TicTacToe_state::get_winner() const { return winner; }

void TicTacToe_state::change_turn() { turn = (turn == 'x') ? 'o' : 'x'; }
...
MCTS_state *TicTacToe_state::next_state(const MCTS_move *move) const {
  ...
  TicTacToe_state *new_state =
      new TicTacToe_state(*this);  // create new state from current
  if (new_state->board[m->x][m->y] == ' ') {
    new_state->board[m->x][m->y] = m->player;  // play move
    new_state->winner =
        new_state->calculate_winner();  // check again for a winner
...
double TicTacToe_state::rollout() const {
  ...
  // Note: dequeue is not very efficient for random accesses but vector is not
  // efficient for deletes
  deque<int> available;
  for (int i = 0; i < 9; i++) {
    if (board[i / 3][i % 3] == ' ') {
  ...
  double res = (curstate->winner == 'x')   ? 1.0
               : (curstate->winner == 'd') ? 0.5
                                           : 0.0;
  delete curstate;
  return res;
}
...
bool TicTacToe_move::operator==(const MCTS_move &other) const {
  const TicTacToe_move &o =
      (const TicTacToe_move &)other;  // Note: Casting necessary
