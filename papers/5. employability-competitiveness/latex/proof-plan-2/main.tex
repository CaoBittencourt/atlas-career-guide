...
\begin{definition}[Time Allocation]
    ...
    \begin{gather}
        ...
        ,
    \end{gather}
    where $\text{ttc}(l)$ is the number of hours to complete a task given its difficulty level.
...
%             ,
%         \end{gather}
%         $\boldsymbol{\mho_q}$ and $\boldsymbol{w_q}$ are entirely determined by responsability bounds $\boldsymbol{\ell_q}$; or, conversely, $\boldsymbol{\mho_q}$ and $\boldsymbol{\ell_q}$ can be written in terms of $\boldsymbol{w_q}$, so that
%         \begin{align}
%             &
%             \max_{
%                 w_{q}^{v}
