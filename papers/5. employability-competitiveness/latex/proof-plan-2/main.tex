...
\begin{observation}[Occupational Reducibility]
    From a practical standpoint, occupations are reducible to their tasks: a ``job'' is nothing but a collection of tasks which have to be executed in a particular time frame.
\end{observation}

And, with this very basic notion, we conclude that if person can perform an occupation's tasks, they can, thereby, be employed in its labor market; as, again, a job is just a collection of tasks. Therefore, one is employable to measure of one's productivity. 

However, how should we assess productivity when speaking of whole occupations? For, though a job is, indeed, a collection of tasks, we do not know exactly \textit{which} tasks, whether simple, or complex, and if so, how complex, constitute what is called ``an occupation''. Morever, this set of tasks of varying complexity and productivity requirements also varies from position to position, even within the same field.
