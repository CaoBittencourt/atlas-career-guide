...
\begin{axiom}[Employer Rationality Axiom, ERA]
    \label{era}
    Employers are rational and will only pay for employees to work on tasks they can accomplish. Additionaly, if labor stratification is allowed, rational employers will split job posts and outsource activities if they expect employees cannot accomplish the whole operation.

    Mathematically, a rational employer's optimization problem is to choose two vectors of $\boldsymbol{w_q}$ employment levels and responsability bounds $\boldsymbol{\ell_q}$ for each of $p_q$ positions and implement a production strategy that maximizes operational output, given the available talent, or expected productivity in the workforce:
    \begin{align}
        \max_{
            p_q,
            \boldsymbol{w_q},
