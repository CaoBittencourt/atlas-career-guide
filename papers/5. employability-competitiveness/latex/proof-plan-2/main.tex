...
\begin{lemma}[Maximum Operational Output Lemma, MOOL]
    ...
    \begin{proof}
        As we are want to derive maximum operational output, throughout this proof we assume
        \begin{gather}
...
\begin{lemma}[Productivity Sufficiency Lemma, PSL]
    ...
    \begin{proof}[Proof 2]
        ...
        Now, binary labor stratification (BS) demands at least one perfectly qualified ``senior'' and a sufficiently qualified ``junior'' for an aggregate output $\mho_{q}^{\text{BS}} > 1$, greater than that of independent production:
        \begin{align}
            \boldsymbol{w_q}(
                \boldsymbol{\tilde{T}_q},
                \boldsymbol{\ell_q}
