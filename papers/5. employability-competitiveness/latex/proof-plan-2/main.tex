...
\begin{axiom}[Occupational Complexity Axiom]
    ...
    \begin{subaxiom}[Moderate Occupational Complexity Axiom, MOCA] With moderate occupational complexity, we assume production can be split into $p$ positions, or job subtypes, each responsible for their own set of tasks with increasing difficulty levels.
    
    So, though the operation, in itself, remains ``indivisible'', it can be split up and outsourced, so long as all its subtasks are completed: it can be ``divided'' if it is, then, ``pieced back together'' with all its parts.
