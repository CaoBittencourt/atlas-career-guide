...
\begin{axiom}[Occupational Complexity Axiom]
    ...
    \begin{subaxiom}[Moderate Occupational Complexity Axiom, MOCA]
        though the operation, in itself, remains an ``indivisible'' task, it can be outsourced, so long as all its subtasks are completed: it can be ``divided'' if it is, then, ``pieced back together'' with all its parts.
    \end{subaxiom}
