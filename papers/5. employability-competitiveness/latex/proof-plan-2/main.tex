...
% proof
\begin{proof}
    We have just concluded an occupation is a collection of complex tasks, which in turn depend on a $\boldsymbol{a_q} := (a_{1}^{q},\dots, a_{m}^{q})$ vector of professional attributes to be accomplished (see Definition \ref{complex_task}). By the Skill Composition Lemma, then,
    \begin{gather}
        \tilde{T}_{k}^{q}
        =
        \int_{0}^{1}
...
\begin{axiom}[Hireability Axiom]
    ...
    [Furthermore,] Any rational employer hires employees by evaluating a hireability statistic, which quantifies potential employees' expected productivity, their educational attainment, and years of experience.
...
\begin{axiom}[Productivity Differentia Axiom, PDA]
    ...
    \begin{gather}
        ...
        ,
    \end{gather}
    for all $r \in \{1, \dots, w_{v}^{q}\}, v \in \{1, \dots, p_q\}, p_q \in \{1, 2, 3, \dots \}$. This means employers do not expected every worker to be perfectly qualified and will adjust their hiring and production strategies accordingly.
...
\begin{lemma}[Maximum Operational Output Lemma, MOOL]
    ...
    \begin{proof}
        ...
        % choosing any $\tilde{w}_{v}^{q} \neq \Omega_{v}^{q}$ implies the proportion of at least one position, say $\tilde{w}_{q}^{r}$, is impacted, and thus aggregate output along with it, either because
        % choosing any $\tilde{w}_{v}^{q} \neq \Omega_{v}^{q} \in [0,1]$ implies the proportion of at least one position, say $\tilde{w}_{q}^{r}$, is impacted, and thus aggregate output along with it, either because
        \begin{align}
            &
