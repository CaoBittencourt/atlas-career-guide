...
% region: abstract 
\begin{abstract}
    In this article, we propose a microeconomic framework to evaluate individuals' employability in efficiently organized labor markets as a function of their capacity on a set of measureable professional attributes. We demonstrate this value coincides precisely with the normalized duration of occupations' tasks their skill set allows them to accomplish, or what we term ``inverse operational output''. We further demonstrate labor market competitiveness can be defined as a complement of this employability metric, given by the percentage of workers from other fields that would be willing and able to compete for occupations' job posts.
\end{abstract}
...
\begin{axiom}[Occupational Complexity Axiom]
    ...
    \end{gather}
    aggregation function, which could result in the same conclusions we demonstrate in this paper, if it is ``well behaved'' enough (e.g. satisfying Inada conditions). For simplicity's sake and mathematical convenience, though, we shall assume weak occupational complexity (WOCA).
\end{axiom}
% endregion
