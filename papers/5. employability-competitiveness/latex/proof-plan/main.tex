...
Maximum Operational Output Lemma (MOOL)
    ...
    So, with $\boldsymbol{\ell_{q}^{*}}$ employers acchieve $\mho_{q}(\boldsymbol{\ell_{q}^{*}}) = w_q$, which is the maximum aggregate output of the previous economic configurations. Note, however, they could also attempt to increase production beyond this level if they reduced the responsabilities of a particular job subtype by setting
    \begin{align}
    \ell_v < \ell_{v}^{*}
    \implies
        \left(
    ...
    Nevertheless, because every worker has the same unitary time allowance, this would also entail that either the missing subinterval of complex tasks $l \in (\ell_{v}, \ell_{v}^{*}]$ would not be produced at all, in which case
    \begin{align}
        \mho_{q}(\boldsymbol{\ell_{q}})
        =
        0
