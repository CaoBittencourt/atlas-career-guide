...
The maximum operational output of any labor market is exactly the number of employees in its workforce:
    ...
    Finally, even with a $\boldsymbol{\omega_{q}^{v}}$ vector of partial time allocations for each worker, at least one difficulty subinterval would have to be neglected to emphasize another, because
    \begin{gather}
        \boldsymbol{1} ^ \top
        \cdot
...
\begin{gather}
    ...
    ,
\end{gather}
in which case maximum-monotonic labor stratification is more effective at safeguarding production, as it requires less worker productivity to yield the same operational output.
