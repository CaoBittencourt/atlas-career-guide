...
Maximum Operational Output Lemma (MOOL)
    ...
    We begin with the most trivial of economic configurations, that of independent production with perfectly qualified workers. Here, each employee devotes their
    \begin{gather}
        \int_{0}^{1}{
            \text{ta}(l)dl
        }
        ...
        1
    \end{gather}
    time allowance to output occupation $q$'s complete operation:
    \begin{gather}
        \int_{0}^{1}{
            \text{ta}(l)dl
        }
    ...
    a binary example, where $\tilde{w}_{q}^{\text{Jr}} \in [0,1]$ is the percentage of less qualified workers (i.e. ``juniors''), and $
    % We begin, again, with a binary example, where $\tilde{w}_{q}^{\text{Jr}} \in [0,1]$ is the percentage of less qualified workers (i.e. ``juniors''), and $
    \tilde{w}_{q}^{\text{Sr}} = 1 - \tilde{w}_{q}^{\text{Jr}} \in [0,1]$, the percentage of perfectly qualified workers (i.e. ``seniors''):
    \begin{align}
    ...
    With this, if $\tilde{w}_{q}^{\text{Jr}} \in [0,1]$ is set to $\tilde{w}_{q}^{\text{Jr}} > \Omega_{q}^{\text{Jr}}$, then
    \begin{align}
        &
        1 - \tilde{w}_{q}^{\text{Jr}}
        <
        ...
        ;
    \end{align}
    and, likewise, if $\tilde{w}_{q}^{\text{Jr}} < \Omega_{q}^{\text{Jr}}$,
    \begin{align}
        &
        1 - \tilde{w}_{q}^{\text{Jr}}
        >
