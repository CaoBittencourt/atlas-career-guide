...
Intuitively speaking, we would say production in a maximally and monotonically stratified labor market is not ``independent'', in the sense that employees do not work on the operation from beginning to end. This means each on them will spend all their time allowance producing a partial operational output, that is a multiple of a difficulty subinterval of complex tasks, which will, in turn, contribute, alonside the partial outputs of other employees, to accomplish the occupational operation in its entirety.
...
Maximum Operational Output Lemma (MOOL)
    % \item Maximum Operational Output Lemma (MOOL)
    % \begin{enumerate}
