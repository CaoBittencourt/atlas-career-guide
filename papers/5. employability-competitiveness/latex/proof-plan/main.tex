...
Maximum Operational Output Lemma (MOOL)
    ...
    \end{align}
    where 
    $\mho_{q}^{v}(\ell_{v},\ell_{r}), \Omega_{q}^{v}(\ell_{v},\ell_{r}), v, r \in \{1, \dots, w_q\}$, with
    \begin{gather}
        \mho_{q}^{v}
        =
        \min
        ...
        ,
    \end{gather}
    are short-hand notations for partial operational output and its inverse, total time allocation.
