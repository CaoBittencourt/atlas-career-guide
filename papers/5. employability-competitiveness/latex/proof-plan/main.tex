...
Maximum Operational Output Lemma (MOOL)
    ...
    We begin with the most trivial of economic configurations, that of independent production with perfectly qualified workers. In this scenario, each employee devotes their unitary time allowance, which coincides with the total time duration of occupation $q$'s entire operation,
    \begin{gather}
        \int_{0}^{1}{
            \text{ta}(l)
            dl
    ...
    With this, we note that, as each worker's time allowance is the same as operations' total time requirements, failing to output any single task by overemphasizing another would nullify the whole operation, when production is independent. Hence, the optimal choice of hours to allocate to any task has to be
    \begin{gather}
        \Omega_{q}^{\ell} 
        \in
        [0,1]
