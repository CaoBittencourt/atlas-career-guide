...
% When we speak of ``employability'', however, it is clear the above constructs, though implied in the concept of employability in some way, are not, by themselves, proper metrics to quantify it. For indeed, the employability of a person is the percentage of jobs for which they could be hired in a particular subset of the labor market. So, whatever employability happens to be mathematically, it is, of course, proportional to occupational similarity, competence, and interchangeability, as these are all implicit in the ``hireability'' part of the definition. Nevertheless, we also know that employability is its own construct, which fails to be encapsulated by other constructs. We cannot, for instance, simply infer that if a person $k$ has $s_{k,q} \in [0,1]$ similarity with occupation $q$ they are, therefore, hireable in precisely $s_{k,q}$ percent of that occupation's job posts, even though the real percentage will be proportional to this similarity. And, likewise, competence coefficients $c_k$ and $c_q \in [0,1]$ are not objectively interpretable in terms of hireability, though, again, they seem to part of the equation. The economic processes that determine employability are just too complicated to be described by any of these metrics alone. And the reason for this is that occupations are not fundamentally reducible to a sum of their attributes: in other words, they are \textit{complex}. Thus, to measure employability we must address the issue of occupational complexity and deduce new coefficients from a coherent theoretical framework of careers.
