...
\begin{enumerate}
    ...
    \item $
              ...
              \ddot{s}_{k,q}
          $
    \item candidate is hireable if $_{0}^{1}[h_{k,q}]$
    \item candidate is hireable for $_{0}^{1}[h_{k,q}] \times h_{k,q}$ percent of jobs
    \item hireability vector: $\boldsymbol{h_{k}}$
    \item hireability matrix: $\textbf{H}$
    \item evaluated hireability vector: $_{0}^{1}[\boldsymbol{h_{k}}]$
    \item evaluated hireability matrix: $_{0}^{1}[\textbf{H}]$
    \item ?percent of hireable jobs vector: $_{0}^{1}[\boldsymbol{h_{k}}]\boldsymbol{h_{k}}$
    \item ?percent of hireable jobs matrix: $_{0}^{1}[\textbf{H}]^{\top}\textbf{H}$
\end{enumerate}
...
\begin{enumerate}
    ...
    \item utility equivalence function $\ddot{u}_{k,q} = \text{ueq}(u(\hat{y}_k), u(\hat{y}_q), ...)$
    \item static utility $u_k$ vs comparative utility $u_{k,q}$
    \item incorporate preferences in the utility function like in the eeq function? (def = 1 for O*NET career
          profiles)
    \item e.g. $\ddot{u}_{k,q} = \text{ueq}(u(\hat{y}_k), u(\hat{y}_q), ...) \times s_{k,q}^{\upsilon}$
\end{enumerate}
...
\begin{enumerate}
    ...
    \item $\Lambda_k = \left(\frac{1}{W}\right) \sum_{q=1}^{n}{_{0}^{1}[h_{k,q}] h_{k,q} w_q}$
    \item alternatively, $\Lambda_k = \left(\frac{1}{W}\right) \sum_{q=1}^{n}{_{0}^{1}[h_{k,q}] s_{k,q}^{\geq} w_q}$
    ...
    \item $\Lambda_{k}^{?} = \left(\frac{1}{W}\right) \sum_{?}{_{0}^{1}[h_{k,q}] h_{k,q} w_q}$
...
\begin{enumerate}
    \item employment levels $w_k, w_q$
    \item total workforce $W = \sum_{q=1}^{n}{w_q}$
    \item $\text{vs}_k = \left(\frac{1}{W}\right) \sum_{q=1}^{n}{_{0}^{1}[\ddot{u}_{q,k}] _{0}^{1}[h_{q,k}] h_{q,k} w_q}$
    \item alternatively, $\text{vs}_k = \left(\frac{1}{W}\right) \sum_{q=1}^{n}{_{0}^{1}[\ddot{u}_{q,k}] _{0}^{1}[h_{q,k}] s_{q,k}^{\geq} w_q}$
    ...
    \item $\text{vs}_{k}^{?} = \left(\frac{1}{W}\right) \sum_{?}{_{0}^{1}[\ddot{u}_{q,k}] _{0}^{1}[h_{q,k}] h_{q,k} w_q}$
    \item ? is the economic aggregation level (e.g. sector, industry, market, role)
...
\begin{enumerate}
    ...
    \item p.s.: define another notation, for this conflicts with macro-flexibility
\end{enumerate}
...
\begin{enumerate}
    \item expected compensation based on market prices $\textbf{p}$ and skill set
          $\boldsymbol{a_k}$
    \item $? = \text{E}[y_k | \boldsymbol{a_k}, \textbf{p}] = \sum_{i=1}^{m}{a_{i}^{k} p_i}$
...
\begin{enumerate}
    \item p.s.: define another notation, for this conflicts with micro-flexibility
...
\begin{enumerate}
    ...
    \item ? = $\frac{
          ...
          $
    \item alternatively, ? = $\frac{ \sum_{i=1}^{m}{a_{i}^{k} \Phi_i} }{
              \sum_{i=1}^{m}{a_{i}^{k}} } $
    \item probably just use versatility as an additional name to refer to
          macro-flexibility, thus avoiding conflicting mathematical notation
    \item macro-flexibility $\Phi_i \rightarrow ?$ (versatility)
...
\begin{enumerate}
    \item use standard factor-analytic notation (I already reserved many latin and greek characters for this reason)
\end{enumerate}
...
\begin{enumerate}
    ...
    \item define a expected value notation (also solves the expected compensation, or skill set value, problem above)
    \item $?g_k = \left( 
        \frac{1}{m_\psi?}
        \right)
        \sum_{\psi?}{
    ...
    \item alternatively, write this coefficient as factor scores (if there is a notation for factor scores)
\end{enumerate}
...
\begin{enumerate}
    \item having defined all the statistical pressupositions for these models in the previous papers, calculate employability and competitiveness coefficients
...
\begin{enumerate}
    ...
    \item calculate which careers are most efficient to aim for in the short run (starting from the average skill set)
    \item calculate which careers are most effective to aim for in the long run (starting from the average skill set)
\end{enumerate}

\subsection{The Career Roadmap: A Mathematical Map to Career Choice and Career Development}
\begin{enumerate}
    \item choose another name for this paper?
    \item The Career Roadmap: Statistical Methods for Optimal Career Choice and Career Development?
    ...
    \item combine all statistical metrics so far as the training recommendation coefficient
    \item calculate which careers are most efficient to aim for in the short run (starting from the average skill set)
    \item calculate which careers are most effective to aim for in the long run (starting from the average skill set)
\end{enumerate}
...
\begin{enumerate}
    \item calculate career types based on the factor model from the EFA paper and the statistics already defined in the previous papers
...
\begin{enumerate}
    ...
    \item aggregate on each taxonomic level defined in the economic taxonomy paper (including the entire economy)
\end{enumerate}
