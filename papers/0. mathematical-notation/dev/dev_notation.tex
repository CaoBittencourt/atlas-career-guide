%SECTION: SETUP
% region: document class
\documentclass{article}

% endregion

% region: packages 
\usepackage{xr}
\usepackage{physics}
\usepackage{amsmath}
% \usepackage{float}
% \usepackage{tabularray}
% \UseTblrLibrary{booktabs}
% \usepackage[
%     backend=biber,
%     style=apa
% ]{biblatex}

% endregion

% region: title 
% Title
\title{
    The Career Atlas:\\
    Mathematical Notation
}
\author{Cao Bittencourt}
\date{\today}

% endregion
\allowdisplaybreaks

%SECTION: PAPER
\begin{document}

\maketitle

\begin{gather}
    % \text{Interchangeability cannot be } I(s(\boldsymbol{a_k},\boldsymbol{a_k}) \text{because I is already the Identity Matrix.}
    % german character ß (esset, eszet, or sharp s) is itself interchangeable with 'ss'
    % thus ß could stand for sufficiently similar / sufficient similarity (ss)
    \boldsymbol{\Lambda} = (\Lambda_{1}, ..., \Lambda_{n}),\\
    \Lambda_k = 
    \Lambda(\boldsymbol{h_{k}}, \boldsymbol{\textbf{\textit{\ss}}_{k}}, \boldsymbol{w}) = 
        \left(
            \frac{1}{W}
        \right)
        \sum_{q=1}^{n}{
            h_{k,q}
            \textit{\ss}_{k,q}
            w_q
            % \textit{\ss}(s(\boldsymbol{a_k},\boldsymbol{a_q}), \tau_k, \tau_q, c_q)
        },\\
    \text{vs}_k = 
    \text{vs}(\boldsymbol{h_{k}}, \boldsymbol{\textbf{\textit{\ss}}_{k}}, \boldsymbol{w}, \boldsymbol{y}) = 
    % \left(
    \left(
        \frac{1}{W}
    \right)
    % \right)
        \sum_{q=1}^{n}{
            h_{k,q}
            \textit{\ss}_{q,k}
            w_q
            u(y_q,y_k)
        }
        ,\\
    \textit{\ss}_{k,q} = 
        \text{eq}(s(\boldsymbol{a_k},\boldsymbol{a_q}), c_q)\\
    u(y_q,y_k) =
    \begin{cases}
        1, \text{if } y_k \geq y_q.\\
        0, \text{otherwise.}
    \end{cases}\\
    % \textit{\ss}_{k,q} = 
    %     \text{eq}(s(\boldsymbol{a_k},\boldsymbol{a_q}), c_q)
    %     \times
    %     \text{eq}(\tau_k, \tau_q, c_q)\\
    W = 
        \sum_{q=1}^{n}{
            w_q
        }\\
    \textbf{\ss} = 
    % \textbf{SS} = 
    \begin{bmatrix}
        \textit{\ss}_{1,1} & \hdots & \textit{\ss}_{n,1}\\
        \vdots & \ddots & \vdots\\
        \textit{\ss}_{1,n} & \hdots & \textit{\ss}_{n,n}\\
    \end{bmatrix} = 
    % \textbf{\ss} = 
    % \textbf{SS} = 
    % \begin{bmatrix}
    %     1 & \hdots & \textit{\ss}_{k,1} & \hdots & \textit{\ss}_{n,1}\\
    %     \vdots & \ddots & \vdots & \vdots & \vdots\\
    %     \textit{\ss}_{1,k} & \hdots & 1 & \hdots & \textit{\ss}_{n,k}\\
    %     \vdots & \vdots & \vdots & \ddots & \vdots\\
    %     \textit{\ss}_{1,n} & \hdots & \textit{\ss}_{k,n} & \hdots & 1\\
    % \end{bmatrix}\\
    \begin{bmatrix}
        1 & \hdots & \textit{\ss}_{k,1} & \hdots & \textit{\ss}_{n,1}\\
        \vdots & \ddots & \vdots & \ddots & \vdots\\
        \textit{\ss}_{1,k} & \hdots & 1 & \hdots & \textit{\ss}_{n,k}\\
        \vdots & \ddots & \vdots & \ddots & \vdots\\
        \textit{\ss}_{1,n} & \hdots & \textit{\ss}_{k,n} & \hdots & 1\\
    \end{bmatrix}\\
    \boldsymbol{\textbf{\textit{\ss}}_{k}} = (\textit{\ss}_{k,1}, ..., \textit{\ss}_{k,n})\\
    \boldsymbol{\textbf{\textit{\ss}}_{q}} = (\textit{\ss}_{q,1}, ..., \textit{\ss}_{q,n})\\
    \gamma_{k} = \left(\frac{1}{m}\right)\sum_{i=1}^{m}{â_{i}^{k}}\\
    \gamma_{k} = \frac{1}{m}\sum_{i=1}^{m}{â_{i}^{k}}\\
    \gamma_{k} = \sum_{i=1}^{m}{\frac{â_{i}^{k}}{m}}\\
    \gamma_{k} = \sum_{i=1}^{m}{â_{i}^{k} / m}\\
    \gamma_{k} = 
    \left(
        \frac{1}{m}
    \right)\sum_{i=1}^{m}{
        \frac{a_{i}^{k}}{\max{a_{j}^{k}}}
    },\\
    \boldsymbol{â_k} = (â_{1}^{k}, ..., â_{m}^{k})\\
    â_{i}^{k} = \frac{a_{i}^{k}}{\max{a_{j}^{k}}}\\
    c_{k} = 
    c(\boldsymbol{a_k}, \gamma_{k}) = 
    \frac{
        \sum_{i=1}^{m}{
            {\"a}_{i}^{k}
            \tilde{a}_{i}^{k}
        }
    }{
        \sum_{i=1}^{m}{
            {\"a}_{i}^{k}
        }
    },\\
    {\"a}_{i}^{k} =
    \text{eq}(â_{i}^{k}, \gamma_{k}) = 
    \text{eq}\left(
        \frac{
            a_{i}^{k}
        }{
            \max{a_{j}^{k}}
        },
        \gamma_{k}
    \right)\\
    c_{k} = 
    c(\boldsymbol{a_k}, \gamma_{k}) = 
    \frac{
        \sum_{i=1}^{m}{
            a_{i}^{k}
            \tilde{a}_{i}^{k}
        }
    }{
        \sum_{i=1}^{m}{
            a_{i}^{k}
        }
    }?,\\
    \boldsymbol{{\"a}_{k}} = ({\"a}_{1}^{k}, ..., {\"a}_{m}^{k})\\
    \boldsymbol{{\"a}_{q}} = ({\"a}_{1}^{q}, ..., {\"a}_{m}^{q})\\
    \boldsymbol{{\"a}_{n}} = ({\"a}_{1}^{n}, ..., {\"a}_{m}^{n})\\
    \textbf{{\"A}} = 
    \begin{bmatrix}
        {\"a}_{1}^{1} & \hdots & {\"a}_{m}^{1}\\
        \vdots & \ddots & \vdots\\
        {\"a}_{1}^{n} & \hdots & {\"a}_{m}^{n}\\
    \end{bmatrix}\\
    \ddot{\tau}_{k,q} = 
    \text{edq}(\tau_{k}, \tau_{q}, c_{q}) = 
    \left\{
        1 + \exp[-
        \frac{
            \tau_{q}
            (1 + \tau_{k} - \tau_{q})
        }{
            1 - c_{q}
        }
        ]
    \right\} ^ {-1}\\
    \textit{\ss}_{k,q} = 
    \ddot{s}_{k,q}
    \times
    \ddot{\tau}_{k,q} = 
    \text{eq}(s(\boldsymbol{a_k},\boldsymbol{a_q}), c_q)
    \times
    \text{edq}(\tau_{k}, \tau_{q}, c_{q})\\
    \text{notation for a binary indicator (bin) operator?}\\
    % ?x?_{k,q} = 
    % h_{k,q} = 
    % {_{0}^{1}}\overset{0.5}{[{\textit{\ss}_{k,q}}]}
    % \times
    % {_{0}^{1}}\overset{0.5}{[{\textit{s}_{k,q}}]} = \\
    % {_{0}^{1}}\overset{0.5}{[{\textit{\ss}_{k,q}}]}
    % \times
    % {_{0}^{1}}\overset{0.5}{[{\textit{s}_{k,q}}]}\\
    % h_{k,q} =\\
    %     {_{0}^{1}\rvert s_{k,q}^{*}} \times
    %     {_{0}^{1}\rvert \textit{\ss}_{k,q}} =\\
    %     \text{bin}(s_{k,q}^{*}) \times
    %     \text{bin}(\textit{\ss}_{k,q}) =\\
    %     \text{bin}(\text{sq}(\boldsymbol{a_k},\boldsymbol{a_q})) \times
    %     \text{bin}(\text{seq}(\boldsymbol{a_k},\boldsymbol{a_q}))
    \text{bin}(x_{k,q}) = 
    \begin{cases}
        1, \text{if } x_{k,q} \geq 0.5.\\
        0, \text{otherwise.}
    \end{cases}\\
    \text{hireability--interchangeability distinction:}\\
    \text{sufficiently qualified (sq) vs sufficiently similar (ss, \ss)}\\
    \text{meets minimum requirements vs same shape}\\
    \text{sq}(\boldsymbol{a_k},\boldsymbol{a_q}) = 
    1 - 
    \sqrt{
        \frac{
            \sum_{i=1}^{m}{
                \ddot{a}_{i}^{q}
                \delta(
                    {a}_{i}^{k},
                    {a}_{i}^{q}
                ) ^ 2
            }
        }{
            \sum_{i=1}^{m}{
                \ddot{a}_{i}^{q}
                \max(
                    100 - {a}_{i}^{q}, 
                    {a}_{i}^{q}
                ) ^ 2
            }
        }
    },\\
    \delta({a}_{i}^{k}, {a}_{i}^{q}) = 
    \max({a}_{i}^{q} - {a}_{i}^{k}, 0)\\
    h_{k,q}
    = h(\text{sq}_{k,q},c_{q})
    = h(\text{sq}(\boldsymbol{a_k},\boldsymbol{a_q}),c_{q}) 
    = \ddot{\text{sq}}_{k,q}
    = \text{eq}\left(
        \text{sq}_{k,q},
        c_{q}
    \right)\\
    \text{to be hireable is: }
    \text{bin}(h_{k,q}, c_{q}) = 
    \begin{cases}
        1, \text{if } h_{k,q} \geq 0.5 ^ {1 - c_{q}}.\\
        0, \text{otherwise}.
    \end{cases}\\
    \text{Attribute macro-flexibility:}\\
    \Phi_{i} = 
    \left(\frac{1}{W}\right)
    \sum_{k=1}^{n}{
        \tilde{a}_{i}^{k}
        w_{k}
    }
\end{gather}

\end{document}