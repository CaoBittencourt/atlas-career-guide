...
\begin{enumerate}
    ...
    \item need to define an axiom to go from x percent of hireability in one job of a class to hireability in x percent of jobs of that class
\end{enumerate}
...
\begin{enumerate}
    ...
    \item $\Phi_k = \frac{ \sum_{i=1}^{m}{a_{i}^{k} \Phi_i} }{
              \sum_{i=1}^{m}{a_{i}^{k}} }$
    \item probably move all the $i,j$ attribute iterators from subscripts to superscripts (this avoids ambiguity and seems to make sense with the vectorized notation)
\end{enumerate}
