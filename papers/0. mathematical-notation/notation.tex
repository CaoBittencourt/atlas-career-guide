...
\begin{gather}
    % a_{i}^{k} = \sum_{\ell=0}^{a_{\text{ub}}}T_{\ell}
...
So, however one decides to interpret skill levels, the conclusion remains the same: to be skilled in an attribute is to be able to perform the activities associated with that attribute. Put simply, the capacity to \textit{act} follows \textit{virtue}, for virtue, itself, is the capacity to act [or for virtue is, itself, the capacity to act].
