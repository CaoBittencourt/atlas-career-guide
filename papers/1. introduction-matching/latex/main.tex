...
matching against human subjects. This was done for four reasons: 1) to ensure
the model's internal consistency; 2) to avoid potential sampling biases; 3)
because matching occupations to occupations is, essentially, the same as
matching individuals to occupations (for they are both defined by competency
vectors); 4) and, finally, because the O*NET data are, themselves, derived from
rigorous labor market surveys, so that career profiles already are
representative of the skills of real people. In other words, even though one of
the model's end goals is to help individuals find the right career, at the
present moment, we can test these methods ``internally", without resourcing to
further empirical research.

Nevertheless, this also means we cannot show most of our results, as the O*NET
...
``sufficient range'' of careers: two highly qualified S.T.E.M. occupations, the
