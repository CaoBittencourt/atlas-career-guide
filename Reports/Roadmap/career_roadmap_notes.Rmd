...
since $p_i = 0$ for most attributes. In addition, it should be noted that in all of these formulations $\delta_i^q$ could be substituted for $\delta_i^q(s_q^*)$, if the desired goal is not to fully maximize matching percentage with a given occupation, but only to reach an arbitrary $s_q^*$ compatibility.
...
In this case, the coefficient becomes:
...
which is an unbounded version, expressing the present value of future "flexible dollars" ($k_f^i p_i \delta_i^q$) obtained from investing $t_i$ time units to close the $\delta_i^q$ competency gap.

Therefore, in this model, individuals discount the expected future value of human capital taking into consideration the time it would take to develop such capital, then compare the present values of different payoffs so as to rationally decide on which competencies to develop.

The advantage of this unbounded formulation is that it is expressed in the variables's original units, and thus it yields the actual estimation of expected "flexible dollars per hour", instead of a dimensionless percentage. However, there are many different ways of constructing the recommendation coefficient, and a dimensionless model can be useful too. One example for the bounded percentage-based metric is:
...
Having understood this, an example of how this similarity-maximizing strategy can be written is:
...
which corresponds to the total time investment necessary to close the individual's competency gap $\delta_i^q(s_q^*,a_i^q,a_i^u)=\max(s_q^*a_i^q - a_i^u ,0)$ in a given attribute to adequately exercise occupation $q$ at the minimum "acceptable" compatibility level $s_q^m$, after adjusting for over-qualification.

In other words, using this economic strategy, individuals will attempt to reach "acceptable" matching percentages with their chosen occupations by developing the most lagging abilities first, up to the optimum point $a_i^u = s_q^m a_i^q$, where minimum "acceptable" similarity score $s_q^m$ is attained.

Of course, it is also possible to provide a middle-ground approach as a linear combination of different strategies. For instance, taking the above functional forms, we could have:
...
\right]
\right\}
+ (1-\pi)[t_i\max(s_q^m a_i^q-a_i^u, 0)]
, \pi \in [0,1]
$$

where $\pi$ is the parameter indicating preference for faster acquisition of "flexible dollars per hour", and $1-\pi$, the preference for faster attainment of minimum "acceptable" professional compatibility, both of which can be defined beforehand, or by user input.

Again, whatever may be the functional form of this competency training recommendation coefficient $\rho_i$, optimal career development should always aim to maximize it at each and every stage of an individual's working life.
