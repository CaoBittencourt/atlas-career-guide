...
where the binary utility function $U(\vb{a_u},\vb{a_k},w_u,w_k)$ indicates whether an economic agent would benifit from switching jobs, based on their current wage $w_u$, and the wages other occupations. We also assume here that their expect payment, if they choose to change jobs, is a $\mathrm{I}(\vb{a_u},\vb{a_k})$ fraction of market wages, so they should be paid in accordance with their interchangeability levels. This way, one can only attain the full salary of a given occupation, if considered 100% interchangeable with that occupation. We could interpret this mechanism as employer's willingness to outsource a portion of a job's activities that could be performed by lower-payed workers.
