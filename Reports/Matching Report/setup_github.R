...
# OUTPUT NAME -------------------------------------------------------------
save_object('R/fun_unique_name.R', file = '/tmp/fun_unique_name.R', bucket = list_bucket.fun, region = 'us-east-2')
...
fun_handler <- function(body, ...){  
  ...
  save_object("db/occupations.csv", file = "/tmp/occupations.csv", bucket = list_bucket.data, region = 'us-east-2')
